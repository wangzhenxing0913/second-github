...
public class RedisClient {
	...
	public void show() {
		/*
		 * KeyOperate(); StringOperate(); ListOperate(); SetOperate();
		 * SortedSetOperate(); HashOperate();
		 */
		me();
	 ...
	 */
	private void me() {
		redis.hset("website", "zhiwang", ".com");
		System.out.println(redis.hget("website", "zhiwang"));
		Map map = new HashMap();
		map.put("三", "1");
		map.put("二", "2");
		redis.hmset("map", map);
		Map<String, String> maps = redis.hgetAll("map");
		redis.hdel("map", "一");
		...
		/*redis.lpush("list1", "8");
		redis.lpush("list1", "9");
		redis.lpush("list1", "10");
		List<String> list = redis.lrange("list1", 1, -1);
		...
		redis.sadd("name1","s1");
		redis.sadd("name1", "12");
		Set<String> names = redis.smembers("name1");
	...
	private void KeyOperate() {
		...
		/*
		 * Iterator t1=keys.iterator() ; while(t1.hasNext()){ Object
	...
	private void HashOperate() {
		...
		Map<String, String> m = redis.hgetAll("hash");
